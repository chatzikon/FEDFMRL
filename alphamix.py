...
class AlphaMixSampling(Strategy):
    ...
    def query(self, n, idxs_prohibited=None):
        ...
        idxs = self.idxs_lb if idxs_prohibited is None else (
                    self.idxs_lb + idxs_prohibited)  ####boolean tensor, True if an image is labeled
        idxs_unlabeled = np.arange(self.n_pool)[~idxs]  ###tensor with indices of unlabeled images

        ulb_probs, org_ulb_embedding = self.predict_prob_embed(self.X[idxs_unlabeled],
                                                               self.Y[idxs_unlabeled])  ###embeddings and probabilities
        # of unlabeled images

        probs_sorted, probs_sort_idxs = ulb_probs.sort(descending=True)  ##sort probabilities of every image

        pred_1 = probs_sort_idxs[:, 0]  ##keep indeces with max probability

        lb_probs, org_lb_embedding = self.predict_prob_embed(self.X[self.idxs_lb],
                                                             self.Y[self.idxs_lb])  ###embeddings and probabilities
        # of labeled images
        ...
        ####gradient calculation for unlabeled
        ...
        ###breaks when alpha cap equals to 1 or if enough candidate images are chosen
        ...
        while alpha_cap < 1.0:
            ...
            print('the number of candidates is ' + str(candidate.sum()))
        ...
        if candidate.sum() > 0:
            ...
            self.writer.add_scalar('stats/alpha_mean_mean', min_alphas[candidate].mean(dim=1).mean().item(),
                                   self.query_count)
            self.writer.add_scalar('stats/alpha_std_mean', min_alphas[candidate].mean(dim=1).std().item(),
                                   self.query_count)
            self.writer.add_scalar('stats/alpha_mean_std', min_alphas[candidate].std(dim=1).mean().item(),
                                   self.query_count)
    ...
    def find_candidate_set(self, lb_embedding, ulb_embedding, pred_1, ulb_probs, alpha_cap, Y, grads):
        ...
        for i in range(self.args.n_label):
            ...
            if emb.size(0) == 0:
                emb = lb_embedding
            anchor_i = emb.mean(dim=0).view(1, -1).repeat(unlabeled_size, 1)  ####mean embedding per class
            ...
            if self.args.alpha_closed_form_approx:
                ...
                embedding_mix = (1 - alpha) * ulb_embed + alpha * embed_i  ###alpha mix
                out, _ = self.model.clf(embedding_mix, embedding=True)
                out = out.detach().cpu()
                alpha = alpha.cpu()

                pc = out.argmax(dim=1) != pred_1  ####boolean tensor, with True if model output changes after alpha mix
            else:
            ...
            alpha[~pc] = 1.  ###keep only alphas with pc==True
            pred_change[pc] = True  ###True if pc==True
    ...
    def learn_alpha(self, org_embed, labels, anchor_embed, alpha, alpha_cap, log_prefix=''):
        ...
        for i in range(self.args.alpha_learning_iters):
            ...
            for b in range(math.ceil(float(alpha.size(0)) / self.args.alpha_learn_batch_size)):
                ...
                opt = torch.optim.Adam([l], lr=self.args.alpha_learning_rate / (
                    1. if i < self.args.alpha_learning_iters * 2 / 3 else 10.))
        ...
        if pred_changed.sum() > 0:
            ...
            self.writer.add_scalar('alpha_change/mean_%s' % (log_prefix), min_alpha[pred_changed].mean().item(),
                                   self.query_count)
            self.writer.add_scalar('alpha_change/std_%s' % (log_prefix), min_alpha[pred_changed].std().item(),
                                   self.query_count)
            self.writer.add_scalar('alpha_change/norm_%s' % (log_prefix), min_alpha[pred_changed].norm(dim=1).mean(),
                                   self.query_count)
            self.writer.add_scalar('alpha_change/clf_loss_%s' % (log_prefix), tot_clf_loss / min_alpha.size(0),
                                   self.query_count)
            self.writer.add_scalar('alpha_change/loss_%s' % (log_prefix), tot_loss / min_alpha.size(0),
                                   self.query_count)
        else:

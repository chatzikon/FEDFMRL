...
def main(total_epochs, evaluate, path, splits, t_round, wd, normalization, n_clients, seed, count, lr, dataset,
         ...
         nclients_div, multiloss, scale, multiloss_type, alpha_wd_factor,  SoTA_comp, batch_size):
    ...
    if evaluate == True:
        evaluate(test_loader, path, mod, normalization, n_classes, depth, alpha_opt, SoTA_comp)
    ...
    best_prec, best_round, model, lr_next, round_n, test_r_loss, test_r_prec = communication_round_train(writer, n_rounds, n_clients, n_classes,normalization,
                                                                                                         mod,  depth,
                                                                               momentum, weight_decay, lr, t_round, save_path, mode, train_loader_list,
                                                                               valid_loader_list, total_epochs, count, coef_t, coef_d, lr_next, checkpoint_prev,
                                                                               extracted_logits, valid_loader_server, common_dataset_size, path_s,
                                                                               alphamix_global, alpha_cap, inverse,case_alpha, alpha_normalization, closed_form_approximation,
                                                                               alpha_opt, alpha_hyperparams, nclients_div, multiloss, scale, multiloss_type,
                                                                               alpha_wd_factor,  SoTA_comp)
...
if __name__ == '__main__':
    ...
    for j in range(iters):
        ...
        main(argument_dict['epochs'], False, argument_dict['path'], splits, argument_dict['t_round'],
             ...
             argument_dict['SoTA_comp'],
             argument_dict['batch_size'])
        count += 1
